...
A expressão ``school[2]`` seleciona o caractere no índice 2 de ``school``, e cria um novo string contendo apenas este caractere. A variável ``m`` faz referência ao resultado.
...
do zero. A letra no índice zero do ``"Luther College"`` é ``L``. Então,
na posição ``[2]`` temos a letra ``t``.
...
A expressão entre parênteses é chamado um **índice**. Um índice
especifica um membro de uma coleção ordenada. Neste caso, a coleção de
caracteres na seqüência. o índice *indica* qual caractere você
...
.. mchoicemf:: test_question8_3_1
   ...
   :feedback_b: Há 2 characteres o mas quantos p's?
...
O que você acha que ``fruit[:]`` significa?
...
A condição do loop é ``position < len(fruit)``, por isso, quando
...
com o índice ``len(fruit)-1``, que é o último caractere no string.
